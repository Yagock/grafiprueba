...
Video de webcam como fondo; objeto 3D animado anclado al marcador.
...
MODEL_SCALE = 0.04
OBJECT_MODE = "bouquet"
WINDOW_TITLE = "RA: ArUco + Animación Lluvia (T=cambiar modelo, ESC=salir)"
...
# ---------------------------------------------------------------------------
# Variables globales para la animación de lluvia
# ---------------------------------------------------------------------------
RAIN_PARTICLES = []
LAST_RAIN_TIME = 0.0
...
# Geometría de Flores
# ---------------------------------------------------------------------------
def draw_flower(scale: float, r: float, g: float, b: float) -> None:
    glColor3f(1.0, 0.9, 0.0)
    glPushMatrix()
    draw_sphere(scale * 0.3)
    glPopMatrix()
    ...
    for i in range(num_petals):
        ...
        draw_sphere(scale * 0.25)
        glPopMatrix()


def draw_bouquet(scale: float) -> None:
    glPushMatrix()
    glTranslatef(0.0, 0.0, scale * 0.5)
    draw_flower(scale, 1.0, 0.2, 0.3)
    glPopMatrix()

    glPushMatrix()
    glTranslatef(-scale * 0.7, scale * 0.4, scale * 0.2)
    glRotatef(30, 0, 1, 0)
    draw_flower(scale * 0.8, 0.4, 0.6, 1.0)
    glPopMatrix()

    glPushMatrix()
    glTranslatef(scale * 0.7, scale * 0.4, scale * 0.2)
    glRotatef(-30, 0, 1, 0)
    draw_flower(scale * 0.8, 1.0, 0.5, 0.0)
    glPopMatrix()

    glPushMatrix()
    glTranslatef(0.0, -scale * 0.6, scale * 0.1)
    glRotatef(20, 1, 0, 0)
    draw_flower(scale * 0.8, 0.9, 0.3, 0.8)
    glPopMatrix()


# ---------------------------------------------------------------------------
# Lógica de Animación de Lluvia
# ---------------------------------------------------------------------------
def spawn_rain(scale: float) -> None:
    """Genera un nuevo grupo de gotas de lluvia aleatorias."""
    global RAIN_PARTICLES
    RAIN_PARTICLES = []
    # Creamos 40 bolitas para la lluvia
    for _ in range(40):
        RAIN_PARTICLES.append({
            "x": random.uniform(-scale * 2.5, scale * 2.5),
            "y": random.uniform(-scale * 2.5, scale * 2.5),
            "z": random.uniform(scale * 3.0, scale * 8.0), # Empiezan en el aire
            "speed": random.uniform(scale * 2.0, scale * 5.0) # Distintas velocidades
        })

def update_and_draw_rain(dt: float, scale: float) -> None:
    """Actualiza la posición de las gotas y las dibuja."""
    global LAST_RAIN_TIME, RAIN_PARTICLES
    current_time = glfw.get_time()
    
    # Cada 5 segundos, iniciamos una nueva lluvia
    if current_time - LAST_RAIN_TIME >= 5.0:
        spawn_rain(scale)
        LAST_RAIN_TIME = current_time
        
    glColor3f(0.0, 0.6, 1.0)  # Color azul claro para el agua
    for p in RAIN_PARTICLES:
        # La gravedad hace que caigan en el eje Z
        p["z"] -= p["speed"] * dt
        
        # Solo dibujamos la gota si no ha tocado el suelo (Z > 0)
        if p["z"] > 0:
            glPushMatrix()
            glTranslatef(p["x"], p["y"], p["z"])
            draw_sphere(scale * 0.04) # Gotitas muy pequeñas
            glPopMatrix()
...
def draw_scene_3d(rvec, tvec, camera_matrix, width, height, mode, scale, dt) -> None:
    ...
    # Dibujamos el objeto central (Ramo, Esfera o Tetera)
    draw_ar_object(mode, scale)
    
    # Actualizamos y dibujamos la animación de la lluvia por encima de los objetos
    update_and_draw_rain(dt, scale)


def main() -> None:
    global OBJECT_MODE, MODEL_SCALE, LAST_RAIN_TIME
    ...
    # Inicializamos el control de tiempo para animaciones
    last_time = glfw.get_time()
    LAST_RAIN_TIME = last_time - 5.0  # Para que llueva inmediatamente al abrir

    while not glfw.window_should_close(window):
        # Calculamos el Delta Time para movimiento suave
        current_time = glfw.get_time()
        ...
        if corners is not None:
            ...
            # Pasamos el dt a nuestra función de dibujado 3D
            draw_scene_3d(rvec, tvec, camera_matrix, w, h, OBJECT_MODE, MODEL_SCALE, dt)
